<script>
    ...
    onMount(async () => {
        ...
        .then(data => {
            if ($orders != data) {
                orders.set(data);
                ongoingOrders = data.filter(order => ["WAITING_ADMIN_APPROVAL", "DELIVERING", "DELIVERED_AND_WAITING_FOR_PICKUP"].includes(order.status))
            }
        }).catch(error => {
            return [];
        });
 ...
 {:else}
    <div class="page-fullwidth">
        <div class="empty-outer">
            <center>
                <div class="empty">
                <p class="gutter-bottom">Your have no ongoing orders.</p>
